...
async def product_info(update: Update, context: CallbackContext):
    data = update.message
    bot = context.bot
    # TODO
    print(f"message photo: {update.message.photo}")
    # photo = bot.getFile(update.message.photo[-1].file_id)
    # file_ = open('product_image', 'wb')
    # photo.download(out=file_)
    file = await context.bot.get_file(update.message.photo[-1].file_id)
    await file.download_to_drive('product_image')
    data = update.message.caption.split(',')
    # upload image to cloudinary
    upload("product_image")
    url, options = cloudinary_url("product_image", width=100, height=150, crop="fill")
    # create new product
    ...
    client.query(
        q.update(
            q.ref(q.collection("Businesses"), context.user_data["sme_id"]),
            {"data": {
...
## CUSTOMER
async def customer_pref(update, context):
    bot = context.bot
    chat_id = update.callback_query.message.chat.id
...
async def post_view_products(update, context):
    bot = context.bot
    chat_id = update.callback_query.message.chat.id
    data = update.callback_query.data
